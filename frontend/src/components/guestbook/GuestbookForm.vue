...
import { createMessage } from "@/api/guestbook";
...
const emit = defineEmits<{
  submitted: [messages: GuestbookMessage[]];
  cancel: [];
}>();
...
async function handleSubmit() {
  ...
  try {
    const res = await createMessage(props.username, {
      content: content.value,
      parent_id: props.parentId ?? undefined,
    });
    toast.success("留言成功");
    content.value = "";
    emit("submitted", res.messages);
  } catch (err) {
    toast.error(err instanceof Error ? err.message : "留言失败");
  } finally {
...
<template>
  ...
  >
    请先<router-link to="/login" class="text-primary hover:text-primary-hover underline">登录</router-link>后再留言
  </div>
