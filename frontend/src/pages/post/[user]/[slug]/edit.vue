...
import { Save, ArrowLeft, Trash2, Pin } from "@lucide/vue";
...
async function loadPost() {
  ...
  try {
    ...
    if (!userStore.isLoggedIn || !userStore.user) {
      ...
      router.replace(`/post/${post.author.github_login}/${post.slug}`);
      return;
    }
...
async function onDelete() {
  ...
  try {
    ...
    toast.success("文章已删除");
    router.push(`/user/${route.params.user}`);
  } catch (err) {
        ...
        <div class="flex flex-col gap-4 pt-3 sm:pt-4 border-t border-border-soft">
          ...
          <div class="flex items-center gap-3">
            <button
              type="submit"
              :disabled="submitting"
              class="inline-flex items-center gap-1.5 px-6 py-2 rounded-cute-sm bg-primary hover:bg-primary-hover text-white text-sm transition-colors disabled:opacity-50"
            >
              <Save :size="14" />
              {{ submitting ? "保存中..." : "保存" }}
            </button>
